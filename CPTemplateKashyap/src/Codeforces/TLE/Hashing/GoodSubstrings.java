package Codeforces.TLE.Hashing;
...
public class GoodSubstrings {
    ...
    public static void main(String[] args) {
        ...
        try {
          ...
          out.println(solve(n,s.toCharArray(),s1.toCharArray(), k,s)); 
    ...
    static long solve(int n, char[] s, char[] arr, int k,String S ){
       long ans=1;
      // HashSet<List<Long>> s1 =new HashSet<>();
       long[] pow_p= new long[n];
       long[] pSum = new long[n];
       long[] pSum1 = new long[n];
       long[] pow_p1= new long[n];
       ...
       pSum1[0]=s[0]-'a'+1;
       for(int i=1;i<n;i++){
        pow_p[i] = (pow_p[i-1]*233l)%mod; 
        pow_p1[i]=(pow_p1[i-1]*37l)%mod1; 
       }
       for(int i=1;i<n;i++){
        pSum[i]= ((pSum[i-1]*233l)%mod+ (s[i]-'a'+1) )%mod;
        pSum1[i]= ((pSum1[i-1]*37l)%mod1+ (s[i]-'a'+1))%mod1;
       }
       ...
       for(int i=0;i<n;i++){
        ...
        for(int j=i;j<n;j++){
            ...
            if(c>k){
                //out.println(S.substring(i,j+1) +" "+c);
                break;
            }
            //out.println(S.substring(i,j+1));
            if(i>0){
                //out.println((pSum[j]+ mod -(pSum[i-1]*pow_p[j-i+1])%mod)%mod +" "+S.substring(i,j+1));
                long h1 =((pSum[j] -(pSum[i-1]*pow_p[j-i+1])%mod)+mod) %mod; 
                long h2 =((pSum1[j] -(pSum1[i-1]*pow_p1[j-i+1])%mod1)+mod1) %mod1; 
                //out.println(h1 +" "+h2 +" "+S.substring(i,j+1));
                //s1.add(new Pair(h1,h2));
                
                a1.add(new Pair(h1,h2));
                
            }else{
                //out.println((pSum[j]+ mod )%mod+" "+ S.substring(i,j+1));
                long h1 = (pSum[j])%mod;
                long h2 = (pSum1[j])%mod1;
                a1.add(new Pair(h1,h2));
            }
       ...
       Collections.sort(a1,(x,y)-> x.a==y.a? Long.compare(x.b,y.b): Long.compare(x.a,y.a));
      // long ans=0l;
