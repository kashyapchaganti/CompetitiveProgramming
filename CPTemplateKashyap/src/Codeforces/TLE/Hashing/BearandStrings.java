...
public class BearandStrings {
    ...
    public static void main(String[] args) {
        ...
        try {
          ...
          out.println(solve(n,s.toCharArray())); 
    ...
    static long solve(int n, char[] s ){
        ...
        pSum[0]= s[0]-'a'+1;
        for(int i=1;i<n;i++){
            pSum[i]= ((pSum[i-1]*prime)%mod + s[i]-'a'+1 + mod)%mod;
        }
        String x= "bear";
        long h1=0;
        for(int i=0;i<4;i++){
            h1=((h1*prime)%mod + mod + x.charAt(i)-'a'+1)%mod;
        }
        long ans=0;
        long h2 = pSum[3];
        ...
        for(int i=1;i+3<n;i++){
            h2= (pSum[i+3]-(pSum[i-1]*pow_p[4])%mod + mod)%mod;
            //out.println(h1 +" "+h2);
            if(h2==h1){
                arr.add(i+3);
            }
       ...
       int j=0;
       for(int i=0;i+3<n;i++){
        if(j<arr.size() && arr.get(j)>=i+3){
            ans+=n-arr.get(j);
        }else{
            
            while(j<arr.size() && arr.get(j)<i+3){
                j++;
            }
            if(j<arr.size() && arr.get(j)>=i+3){
                ans+=n-arr.get(j);
