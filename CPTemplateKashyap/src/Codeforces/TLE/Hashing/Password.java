...
public class Password {
    ...
    public static void main(String[] args) {
        ...
        try {
          ...
          //  solve(n,s); // even this is correct 
            solve2(n,s);
    ...
    static void solve2(int n , String s){
        long prime =233l;
        long[] pow_p= new long[n];
        //out.println(n);
        pow_p[0]=1l;
        for(int i=1;i<n;i++){
            pow_p[i]=(pow_p[i-1]*prime)%mod;
        }
        long[] pSum = new long[n];
        pSum[0] = s.charAt(0)-'a'+1;
        for(int i=1;i<n;i++){
            pSum[i]=((pSum[i-1]*prime)%mod+s.charAt(i)-'a'+1)%mod;
        }
        long max=-1l;
        int j=n-1;
        ...
        for(int i=0;i<n &&j>0;i++,j--){
            long h1 = pSum[i];
            long h2 = (mod+pSum[n-1]-((pSum[j-1]*pow_p[i+1])%mod))%mod;
            //out.println(h1 +" "+h2+ s.substring(0, i+1)+ s.substring(j));
            if(h1==h2){
        ...
        while(st<=e){
            int m = st+(e-st)/2;
            if(rabinkarp(arr.get(m), arr,  s, n,pSum,pow_p)){
                max=arr.get(m);
                st=m+1;
            }else{
                e=m-1;
        ...
        if(max==-1l){
            ...
            out.println(s.substring(0,(int)max+1));
        }
        
    }
    static boolean rabinkarp(int m , List<Integer> arr, String s, int n,long[] pSum,long[] pow_p){
        
        long h1 =pSum[m];
        int c=1;
        for(int i=1;i<n && i+m<n;i++){
            long h2 = (mod+pSum[i+m]-(pSum[i-1]*pow_p[m+1])%mod)%mod;
            //out.println(h1+" "+h2);
            if(h1==h2){
               // out.println("Ho");
                c++;
    ...
    static void solve(int n , String s){
        ...
        for(int i=0;i<n &&j>0;i++,j--){
            ...
            if(h1==h2){
                ...
                int k= n-2;
                while(k>=i+1){
                    long h3 = (pSum[k]-((pSum[k-(i)-1]*pow_p[i+1])%mod)+mod)%mod;
                    if(h3==h1){
        ...
        if(max==0){
            ...
            out.println(s.substring(0,(int)max));
        }
