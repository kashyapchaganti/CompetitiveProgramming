...
class OracsandModels {
    ...
    static void solve(int n , int[] nums){
        ...
        for(int i=1;i<=n;i++){
            ...
            max= Math.max(max,check(dp,i,nums,n));
        }
        out.println(max);
    }
    static int check(int[] dp, int i, int[] nums,int n){
        if(i==nums.length){
            return 1;
        }
        if(dp[i]!=-1){
        ...
        int max=1;
        //max=Math.max(max, check(dp,i+1,nums,n));
        
            for(int j=i*2;j<=n;j=j+i){
                if(nums[j]>nums[i]){
                    int a = 1+ check(dp,j,nums,n);
                    max=Math.max(max,a);
                }
            }
