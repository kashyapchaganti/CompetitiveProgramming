...
public class CoinsAtcoder {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            for(int i=0;i<n;i++){
               ...
               double a =solve(n,nums);
                out.println(String.format("%.9f", a));
    ...
    static double solve(int n , double[] nums){
        double[][] dp = new double[n][n];
        for(double[] x:dp){
            Arrays.fill(x,-1);
        }
        return check(dp,0,0,nums,n);
        
        
    }
    static double check(double[][] dp, int i, int k,  double[] nums, int n){
        if(i==n){
            if(k>n-k){
                return 1;
            }else{
        ...
        double a= (double)(nums[i])*(double)(check(dp,i+1,k+1,nums,n));
        double b = (double)(1.0-nums[i])*(double)(check(dp,i+1,k,nums,n));
