...
public class Boredom {
    ...
    static void solve(int n , int[] nums){
        ...
        out.println(check(dp,0,nums,n));
    }
    static long check(long[] dp, int i, int[] nums,int n){
        if(i>=n ){
            return 0;
        }
        if(dp[i]!=-1){
       ...
       // long a = (long)(i*nums[i])+check(dp,i+2,nums,n);
        long max=0;
        int x= bs(i, nums,n);
        int x1 = bs(x,nums,n);
       // out.println(x+" "+x1);
       //1133
        if(x!=n && nums[x]==nums[i]+1){
            long a = (long)(((x-1)-i+1)*(long)nums[i])+check(dp,x1,nums,n);
            max=Math.max(max,a);
        }else{
            long a = (long)(((x-1)-i+1)*(long)nums[i])+check(dp,x,nums,n);
            max=Math.max(max,a);
        }
        ...
        long b=check(dp,x,nums,n);
        return dp[i]= Math.max(max,b);
    }
