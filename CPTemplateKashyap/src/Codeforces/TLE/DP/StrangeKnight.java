...
public class StrangeKnight {
    ...
    static long solve(int n , int[][] nums){
        long[][][] dp = new long[256][256][nums.length];
        for(long[][] x:dp){
            for(long[] x1: x){
                Arrays.fill(x1,-1);
            }
        ...
        return check(dp,0,0,0,nums,n,255,255);
        
        
    }
    static long check(long[][][] dp, int i, int j, int idx, int[][] nums, int n,int n1, int m1){
       if(i==0 && j==0 && idx==n){
            return 1;
       }
       if(idx==n ){
       ...
       long a=0;
       int[][] vis = new int[n1][m1];
       for(int k1=i;k1-i<=nums[idx][0];k1++){
        for(int k2=j;k2-j<=nums[idx][1];k2++){
            if(k1<n1 && k2<m1){
                vis[k1][k2]=1;
                a=(a+check(dp,k1,k2,idx+1,nums,n,n1,m1))%mod;
            }
            
        }
       }
       for(int k1=i;i-k1<=nums[idx][0];k1--){
        for(int k2=j;k2<=nums[idx][1];k2--){
            if(k1<n1 && k2<m1 && k1>=0 && k2>=0 && vis[k1][k2]==0){
                a=(a+check(dp,k1,k2,idx+1,nums,n,n1,m1))%mod;
            }
            
        }
       }
       return dp[i][j][idx]=a%mod;
