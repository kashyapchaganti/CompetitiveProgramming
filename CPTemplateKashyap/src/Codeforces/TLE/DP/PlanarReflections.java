...
public class PlanarReflections {
    ...
    static long solve(int n, int k){
        ...
        long[][][] dp = new long[1002][1002][2];
        for(long[][] x: dp){
            for(long[] x1:x)
            Arrays.fill(x1,-1);
        }
        return check(dp, 1,k,1,n);
    ...
    static long check(long[][][] dp, int i, int k,int dir, int n){
        if(i==0 && k>=1 || i==n+1 && k>=1){
        ...
        long a=0;
        if(dir==1){
            a= (a+check(dp,i+1,k,1,n))%mod;
            a=(a+check(dp,i-1,k-1,0,n))%mod;
        }else if(dir==0){
            a=(a+check(dp,i-1,k,0,n))%mod;
            a=(a+check(dp,i+1,k-1,1,n))%mod;
        }
        
        
        return dp[i][k][dir]=a%mod;
