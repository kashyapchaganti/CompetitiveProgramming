...
public class DimaandBadXor {
    ...
    static void solve(int n, int m, int[][] nums){
        
        Pair[][] dp = new Pair[n][1024];
        
        Pair x = check(dp, 0,0,nums,n,m);
        if(x==null || x!=null &&  x.a==-1){
            out.println("NIE");
        }else{
            int xor=0;
            out.println("TAK");
            for(int i=0;i<n;i++){
                out.print(dp[i][xor].b+1 +" ");
                xor^=nums[i][dp[i][xor].b];
            }
        }
    ...
    static Pair check(Pair [][] dp, int i, int k, int[][] nums, int n , int m ){
        if(i==n){
            if(k>0){
                return new Pair(1,n);
            }else{
        ...
        for(int j=0;j<m;j++){
            Pair x= check(dp, i+1,nums[i][j] ^k,nums,n,m);
            if(x!=null && x.a==1){
                return dp[i][k]=new Pair(1,j);
            }
        }
        return dp[i][k] ;
