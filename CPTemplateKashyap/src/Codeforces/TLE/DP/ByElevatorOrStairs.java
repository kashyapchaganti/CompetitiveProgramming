...
public class ByElevatorOrStairs {
    ...
    static void solve(int n, int c, long[] nums,long[] arr,long[] dis){
        
        Pair[][] dp = new Pair[n][2];
        Pair x= check(dp, 1,0,nums,arr,c,n);
       // out.println()
        int f=0;
        int choice=0;
        for(Pair[] y: dp){
    ...
    static Pair check(Pair[][] dp, int i, int f, long[] nums, long[] arr,int c,int n){
        if(i==n){
            return new Pair(i,0,-1);
        }
        if(dp[i][f]!=null){
            return dp[i][f];
        }
        // min path from i to i+1 via lift or stairs 
        long min =(long)(1e9);
        Pair x;
        long a = nums[i]+ check(dp,i+1,0,nums,arr,c,n).w;
        min= a;
        x=new Pair(i,a,0);
        if(f==1){
            long b = arr[i]+ check(dp,i+1,1,nums,arr,c,n).w;
            if(min>b){
                min= Math.min(min,b);
                x=new Pair(i,b,1);
            }
            ...
            long b= arr[i] + c+check(dp,i+1,1,nums,arr,c,n).w;
            if(min>b){
                min= Math.min(min,b);
                x=new Pair(i,b,2);
            }
    ...
    static class Pair{
        ...
        long w;
        int  c;
        public Pair(int r, long w,int c){
            this.r=r;
            this.w=w;
            this.c=c;
        }
