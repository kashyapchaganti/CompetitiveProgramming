...
public class ArrayWalk {
    ...
    static long solve(int n, int k, int z, long[] nums){
        long[][] dp = new long[n+1][6];
        for(long[] x: dp){
            Arrays.fill(x,-1l);
        }
        return check(dp,0,k,z,nums);
        
        
    }
    static long check(long[][] dp, int i, int k, int z, long[] nums){
        if(k==0 && z>=0 && i>=0 && i<nums.length){
            return nums[i];
        }
        if(z<0 || i>=nums.length || i<0 || k<=0){
            return (long)(-1e9);
        }
        if(dp[i][z]!=-1l){
            return dp[i][z];
        }
        // notice that k is constant for both the choices 
        long pick = (long)nums[i] + (long)(check(dp, i+1,k-1,z,nums));
        long notPick = (long)nums[i]+ (long)(check(dp, i-1,k-1,z-1,nums));
       // out.println(pick +" "+notPick);
       return  dp[i][z]= Math.max(notPick, pick);
