...
public class KnapsackTwo {
    ...
    static void solve(int n ,int w, int[] val, int[]wt){
        long[][] dp = new long[n][(int)(1e5+2)];
        for(long[] x:dp){
            Arrays.fill(x,-1l);
        }
        for(int i=(int)(1e5);i>=0;i--){
            long a =check(dp,0,i,w,val,wt,n);
            if(a<(long)(2e9+7) && a<=w){
                out.println(i);
                break;
            }
    ...
    static long check(long[][] dp, int i, int v, int w,int[] val, int[] wt, int n){
        if(i==n && v==0){
            return 0l;
        }
        if(i>=n || v<0){
        ...
        long pick = wt[i]+check(dp,i+1,v-val[i],w,val,wt,n);
        long notPick = check(dp,i+1,v,w,val,wt,n);
        return dp[i][v]=Math.min(pick,notPick);
