...
public class KnapSack {
    ...
    static long solve(){
        long[][] dp = new long[n][(1e6)]
        
        
    }
    static long check(long[][] dp, int i, int w, long[][] nums){
        if(i==nums.length && w>=0){
            return 0l;
        }
        if(i==nums.length){
            return (int)(-1e9);
        ...
        long pick = nums[i][1]+ check(dp,i+1,w-(int)nums[i][0],nums);
        long notPick = check(dp,i+1,w,nums);
