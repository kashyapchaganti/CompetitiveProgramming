...
public class RoadOptimization {
        ...
        try {
           ...
           // long[] dis = new long[n];
               ...
               // out.println(Arrays.toString(nums));
                out.println(solve(n,l,k,nums,arr));
                /*for(int i=0;i<n;i++){
                    out.print(dis[i] +" ");
                }*/
    ...
    static long solve(int n, int l, int k, long[] nums, long[] arr){
        ...
        return (nums[1]-nums[0])*arr[0] + check(dp,1,0,k,n,arr,nums);
    }
    static long check(long[][][] dp, int i, int j,  int k, int n , long[] arr , long [] nums){
        if(i==n && k>=0){
            return 0;
        }
        if(i>=n || k<0){
        ...
        long min =(long)(1e9);
        long a = (nums[i+1]-nums[i]) * arr[j] + check(dp, i+1,j,k-1,n,arr,nums);
        min =Math.min(min,a);
            
        
        long b = (nums[i+1]-nums[i]) * arr[i] + check(dp, i+1,i,k,n,arr,nums);

        return dp[i][j][k]=Math.min(min,b);
