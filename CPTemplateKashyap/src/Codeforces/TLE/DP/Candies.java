...
public class Candies {
    ...
    static void solve(int n ,int k, int[] nums){
        long[][] dp = new long[n][k+1];
        for(long[] x:dp){
            Arrays.fill(x,-1l);
        }
        out.println(check(dp,0,k,nums,n));
        
        
    }
    static long check(long[][] dp, int i, int sum, int[] nums, int n){
        if(i==n && sum==0){
            return 1l;
        }
        if(i>=n || sum<0){
        ...
        long a = 0l;
        for(int k=0;k<=nums[i] && sum-k>=0;k++){
            a= (a+check(dp,i+1,sum-k,nums,n))%mod;
        }
        return dp[i][sum]=a%mod;
