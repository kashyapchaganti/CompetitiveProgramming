...
public class JustEatIt {
    ...
    static boolean solve(int n, long[] nums){
        ...
        long max1 =0,min1=(long)(1e9),max2=0,min2=(long)(1e9);
        for(int i=0;i<n;i++){
            max1= Math.max(max1, psum[i]);
            min1= Math.min(min1,psum[i]);
        }
        for(int i=1;i<=n;i++){
            max2= Math.max(max2, psum[i]);
            min2= Math.min(min2,psum[i]);
        }
        return Math.max(Math.abs(max2-min2),Math.abs(max1-min1))<psum[n];
