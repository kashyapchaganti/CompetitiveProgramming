...
public class x339D {
        ...
        try {
            ...
            while(m-->0){
                ...
                int z= sc.nextInt();    
                    ...
                    sg.update(0, 0, n-1, z, y-1,0);
                
                    out.println(sg.arr[0]);
    ...
    static class SGT{
        ...
        public void build(int idx, int i, int j,long[] nums, int lvl ){
            if(i==j){
                arr[idx]=nums[i];
                l=lvl;
                return;
            ...
            int mid = (i+j)/2;
            build(2*idx+1, i,mid,nums,lvl+1);
            build(2*idx+2, mid+1,j,nums,lvl+1);
            arr[idx]= operation(arr[2*idx+1],arr[2*idx+2],lvl,l);
        }
        public void update(int idx, int i, int j, int val, int idx1,int lvl){
            if(i==j){
                arr[idx]=val;
                return ;
            }
            int mid = (i+j)/2;
            if(mid<idx1){
                update(2*idx+2,mid+1,j,val,idx1,lvl+1);
                
            }else{
                update(2*idx+1,i,mid,val,idx1,lvl+1);
            }
            arr[idx]= operation(arr[2*idx+1],arr[2*idx+2],lvl,l);
        }
    ...
    static long operation(long a, long b, int lvl, int l){
        // out.println(l +" hiii");
        if(l%2==0){
            return lvl%2==0? a ^ b: a|b;
        }else{
            return lvl%2==0? a | b: a^b;
        }
    }
