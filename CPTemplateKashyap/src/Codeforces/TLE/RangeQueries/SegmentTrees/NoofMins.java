...
public class NoofMins {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (System.getProperty("ONLINE_JUDGE") == null && !ONLINE_JUDGE) {
                FileOutputStream output = new FileOutputStream("src/Codeforces/TLE/RangeQueries/output.txt");
                PrintStream out = new PrintStream(output);
                System.setOut(out);
 
                InputStream input = new FileInputStream("src/Codeforces/TLE/RangeQueries/input.txt");
                System.setIn(input);
            }
        ...
        try {
            ...
            // out.println(Arrays.toString(sg.arr));
            while(m-->0){
                int x= sc.nextInt();
                int y= sc.nextInt();
                int z= sc.nextInt();    
                if(x==1){
                    sg.update(0, 0, n-1, z, y);
                }else{
                    Pair z1 =sg.query(0,0,n-1,y,z-1);
                    out.println(z1.r +" " +z1.c);
    ...
    static class SGT{
        ...
        public void build(int idx, int i, int j,long[] nums ){
            if(i==j){
                arr[idx]=new Pair(nums[i],1);
                return;
            }
            int mid = (i+j)/2;
            build(2*idx+1, i,mid,nums);
            build(2*idx+2, mid+1,j,nums);
            arr[idx]= operation(arr[2*idx+1],arr[2*idx+2]);
        }
        public void update(int idx, int i, int j, int val, int idx1){
            if(i==j){
                arr[idx]=new Pair(val,1l);
                return ;
            }
            int mid = (i+j)/2;
            if(mid<idx1){
                update(2*idx+2,mid+1,j,val,idx1);
                
            }else{
                update(2*idx+1,i,mid,val,idx1);
            }
            arr[idx]= operation(arr[2*idx+1],arr[2*idx+2]);
        }
        public Pair query(int idx, int i, int j, int l, int r){
            if(i>=l && r>=j){ // complete overlap
                return arr[idx];
            }
            if(i>r || j<l){ // disjoint
                return new Pair((long)(1e9),0l);
            }
            // partial
            int mid = (i+j)/2;
            Pair x;
            if(mid<l){
                x= query(2*idx+2,mid+1,j,l,r);
            }else if(mid>r){
                x= query(2*idx+1,i,mid,l,r);
            }else{
                x= operation(query(2*idx+1,i,mid,l,r) ,query(2*idx+2,mid+1,j,l,r));
            }
            return x;
    ...
    static Pair operation(Pair a, Pair b){
        if(a.r<b.r){
            return new Pair(a.r,a.c);
        } else if(a.r>b.r){
            return new Pair(b.r,b.c);
        }
        return new Pair(a.r, a.c+b.c);
    }
