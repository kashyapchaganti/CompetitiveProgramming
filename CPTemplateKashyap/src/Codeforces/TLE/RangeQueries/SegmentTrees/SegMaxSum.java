...
public class SegMaxSum {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            while(m-->0){
                ...
                Pair z1 =sg.query(0,0,n-1,0,n-1);
                out.println(z1.c);
            }
    ...
    static class SGT{
        ...
        public void build(int idx, int i, int j,long[] nums ){
            if(i==j){
                arr[idx]=new Pair(nums[i],Math.max(0,nums[i]),nums[i],nums[i]);
                return;
            }
            int mid = i+(j-i)/2;
        ...
        public void update(int idx, int i, int j, int val, int idx1){
            if(i==j){
                arr[idx]=new Pair(val,Math.max(val,0));
                return ;
            }
            int mid = i+(j-i)/2;
            if(mid<idx1){
    ...
    static Pair operation(Pair a, Pair b){
        Pair x;
        x.sum= a.sum + b.sum;
        x.max= Math.max(x.sum, Math.max(x.pSum+))
    }
    static class Pair{
        long max;
        long sum;
        long pSum;
        long sSum;
        public Pair(long sum,long max, long pSum, long sSum){
            this.max=max;
            this.sum=sum;
            this.pSum= pSum;
            this.sSum= sSum;
