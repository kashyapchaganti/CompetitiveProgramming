...
public class FirstElement {
        ...
        try {
            ...
            while(m-->0){
                ...
                if(x==1){
                    ...
                    out.println(sg.query(0,0,n-1,y,z));
    ...
    static class SGT{
        ...
        public int query(int idx, int i, int j, long val, int k){
            if(i==j && arr[idx]>=val){
                return i;
            }
            if(i>=j){
            ...
            int m= (i+(j-i)/2);
            int a= -1;
            if(k<=m && arr[2*idx+1]>=val){
                a= query(2*idx+1,i,m,val,k);
                if(a!=-1){
                    return a; 
                }
            }
            if(arr[2*idx+2]>=val){
                int b =query(2*idx+2,m+1,j,val,k);
            return b;
            }
            return -1;
