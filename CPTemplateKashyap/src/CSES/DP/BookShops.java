...
class BookShops {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            long[][] dp = new long[n+1][(int)(1e5+2)];
            for(long[] x: dp){
                Arrays.fill(x,-1l);
            }
            ...
            out.println(check(dp,0,sum,arr,pages,n));
    ...
    static long check(long[][] dp, int i, int sum,int[] arr, int[] pages,int n){
        if(i==n && sum>=0){
            return 0l;
        }
        if(sum<0 || i>=n){
        ...
        long pick= (long)(pages[i]) + check(dp,i+1,sum-arr[i],arr,pages,n);
        long notpick = check(dp,i+1,sum,arr,pages,n);
