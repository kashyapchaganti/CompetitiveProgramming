...
class Projects {
    ...
    static long solve(int n , long[][] nums){
        Arrays.sort(nums,(a,b)-> Long.compare(a[0], b[0]));
        long[] dp = new long[n];
        Arrays.fill(dp,-1);
        
        return check(dp,0,nums,n);

    }
    static long check(long[] dp, int i, long[][] nums, int n){
        if(i>=n){
            return 0l;
        }
        ...
        int next = bs(i,nums[i][1],nums);
       // out.println(i +" "+nums[i][2]);
        long pick = nums[i][2]+ check(dp,next,nums,n);
        long notPick = check(dp,i+1,nums,n);
