...
class CoinCombinations {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            for(long[] y:dp) Arrays.fill(y,-1);
            long[] nums = new long[x.size()];
            int j=0;
            for(long y: x) nums[j++]=y;
            
            arr = new ArrayList<>();
            for(int i=0;i<n;i++){
                //long s1=0;
                
                arr.add((int)Math.ceil(s/nums[i]));
            }
            //out.println(arr);
    ...
    static long check(long[][] dp, int i, long s, long n, long[] nums){
        ...
        long a =0;
        a=(a+check(dp,i+1,s,n,nums))%mod; 
        long y= s;
        for(int k=0;k<arr.get(i);k++ ){
            y+=nums[i];
            a= (a+check(dp,i+1,y,n,nums))%mod;
        }
