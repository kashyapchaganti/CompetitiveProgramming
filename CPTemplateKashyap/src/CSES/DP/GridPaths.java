...
class GridPaths {
    ...
    static int check(int[][] dp, int i, int j, String[] grid){
        if( i>=grid.length || j>= grid.length || grid[i].charAt(j)=='*'){
            return 0;
        }
        ...
        int right = (check(dp,i,j+1,grid))%mod;
        int down = (check(dp,i+1,j,grid))%mod;
