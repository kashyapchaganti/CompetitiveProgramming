...
class ArrayDescription {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if(arr[0]==0){
                for(int i=1;i<=t;i++){
                    a=(a+(check(dp,1,i,t,arr)))%mod;        
                }
                out.println(a);
            }else{
                out.println(check(dp,1,arr[0],t,arr));
            }
    ...
    static int check(int[][] dp, int i, int prev, int t, int[] nums){
        if(i==nums.length){
        ...
        if(nums[i]!=0){
            if(Math.abs(nums[i]-prev)<=1){
                a= (a+check(dp,i+1,nums[i],t,nums))%mod;
            }
        }else{
            for(int k=1;k<=t;k++){
                if(Math.abs(prev-k)<=1){
                    //System.out.println(k);
                    a= (a+check(dp,i+1,k,t,nums))%mod;
                }
            }
        }
