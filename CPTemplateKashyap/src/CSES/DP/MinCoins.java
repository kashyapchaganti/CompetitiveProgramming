...
class MinCoins {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            long ans =check(dp,0,0,(long)s,nums);
            out.println(ans==(1e9+7)? -1: ans);
    ...
    static long check(long[][] dp, int i, long s, long n, long[] nums){
        if(s==n && i==nums.length){
            return 0;
        }
        if(s>n || i>= nums.length){
            return (long)(1e9+7);
        ...
        long a =0,min=(long)(1e9+7);
        long notPick = check(dp, i+1,s,n,nums);
        ...
        for(int k=0;k<preSum.size();k++){
            a= k+1+check(dp,i+1,preSum.get(k),n,nums);
            min=Math.min(a,min);
        }
        min= Math.min(min, notPick);
        return dp[i][(int)s]=min;
