...
class MovieFestivalII {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            Arrays.sort(nums, (a,b)-> a[1]==b[1] ? Long.compare(a[0],b[0]):Long.compare(a[1],b[1]));
            TreeSet<Pair> s =new TreeSet<>((a,b)-> a.v==b.v? Long.compare(a.idx,b.idx):Long.compare(a.v,b.v));
            long c=0;
         
            for(int i=0;i<n;i++){
                if(s.lower(new Pair(nums[i][0],i))!=null || s.size()<k){
                    
                    if( s.size()<k){
                        if(s.lower(new Pair(nums[i][0],i))!=null){
                            Pair x =s.lower(new Pair(nums[i][0],i));
                            s.remove(x);
                        }
                        s.add(new Pair(nums[i][1],i));
