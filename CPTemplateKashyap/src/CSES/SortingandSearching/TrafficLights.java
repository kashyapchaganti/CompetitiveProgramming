    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            while(n-->0){
                int add =sc.nextInt();
                lights.add(add);
                int a= lights.lower(add); // lower bound
                int b= lights.higher(add); // upper bound
                dist.put(b-a,dist.get(b-a)-1);
                if(dist.get(b-a)<=0) // since we are inserting "add" in btw lower and upper bound we need to delete the dist
                dist.remove(b-a); 
                dist.put(add-a,dist.getOrDefault(add-a,0)+1);
                dist.put(b-add,dist.getOrDefault(b-add,0)+1);
                
                out.println(dist.lastKey());
            }
