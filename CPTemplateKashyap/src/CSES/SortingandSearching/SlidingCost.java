   ...
   static PriorityQueue<Long> minHeap ,maxHeap;
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            minHeap= new PriorityQueue<>((a,b)-> Long.compare(a,b));
            maxHeap = new PriorityQueue<>((a,b)-> Long.compare(b,a));
            for(int i=0;i<n;i++){
                long v = nums[i];
                insert(nums[i]);
                if(i+1>=k){
                    long median =median();
                    // out.println(lsum+" "+rsum);
                    System.out.print(Math.abs(median*maxHeap.size()-lsum) + Math.abs(rsum-median*minHeap.size() )+" ");
                    if(nums[s]<=maxHeap.peek()){
                        maxHeap.remove(nums[s]);
                        lsum-=nums[s];
                    }else{
                        minHeap.remove(nums[s]);
                        rsum-=nums[s];
                    }
                    s++;
                    rebalance();
    ...
    static void insert(long v ){
        if(maxHeap.size()==0 || maxHeap.peek()>= v){
            lsum+=v;
            maxHeap.add(v);
        }else{
            rsum+=v;
    ...
    static boolean check(int k, int n, long[] nums ,long m){
        long prev= 0;
        int c=0;
        for(int i=0;i<n;i++){
            if(prev+nums[i]>m){
