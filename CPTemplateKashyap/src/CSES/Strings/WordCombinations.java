...
class WordCombinations {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (System.getProperty("ONLINE_JUDGE") == null && !ONLINE_JUDGE) {
                FileOutputStream output = new FileOutputStream("src/CSES/Strings/output.txt");
                PrintStream out = new PrintStream(output);
                System.setOut(out);
 
                InputStream input = new FileInputStream("src/CSES/Strings/input.txt");
                System.setIn(input);
            }
    ...
    static long solve(int n, int t, String[]arr,String s){
        ...
        long[] p = new long[t];
        pow_p[0]=1l;
        pSum[0]= s.charAt(0)-'a'+1;
        for(int i=1;i<n;i++){
            pow_p[i]=(pow_p[i-1]*233l)%mod;
        }
        for(int i=1;i<n;i++){
            pSum[i]= ((pSum[i-1]*233l)%mod + s.charAt(i)-'a'+1)%mod;
        }
        for(int i=0;i<t;i++){
            p[i]= getHash(arr[i].toCharArray());
        }
        return check(dp,0,p,pSum,pow_p, n,t,arr);
    }
    static long getHash(char[] s){
        long h=0l;
        long p = 233l;
        for(int i=0;i<s.length;i++){
            h= ((h*p)%mod + (s[i]-'a'+1))%mod;
        }
        return h;
    }
    static long check(long[] dp, int j, long[] p1, long[] pSum, long[]pow_p, int n, int t,String[] arr){
        if(j==n){
            return 1l;
        }
        if(j>n){
        ...
        for(int i=0;i<t;i++){
            ...
            long h1=0l;
            if(j>0 && (j+(n1-1))<n){
                 h1 = (mod+pSum[j+(n1-1)]- (pSum[j-1]*pow_p[n1])%mod)%mod;
                 //out.println(h1+" "+p1[i]);
            }else if((j+(n1-1))>=n){
                continue;
            }else{
            ...
            if(h1==p1[i]){
                ...
                a=(a+check(dp,j+n1,p1,pSum,pow_p,n,t,arr))%mod;
            }
