...
class FindingPeriods {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            pow_p[0]=1;
            for(int i=1;i<n;i++){
                pow_p[i]=(pow_p[i-1]*31)%mod;
            }
            for(int i=0;i<n;i++){
                int j=i+1;
                boolean f= false;
                while(j<n){
                    int len = Math.min(i+1,Math.abs(n-j));
                    if(len==0) {
                        break;
                    }
                    //out.println(preSum[len-1]+" "+(mod+preSum[j+len-1]-((preSum[j-1])*pow_p[len])%mod)%mod +" "+s.substring(j,j+len));
                    if(len-1>=0 && preSum[len-1]==(mod+preSum[j+len-1]-((preSum[j-1])*pow_p[len])%mod)%mod){
                        j+=len;
                        f=true;
                    }else{
                        f=false;
    ...
    static void getHash(String s, int n, long[] preSum){
        if(n==0){
            return;
        }
        preSum[0]=(s.charAt(0)-'a')+1;
        
        
        for(int i=1;i<n;i++){
            preSum[i]=(((preSum[i-1])*31)%mod+(s.charAt(i)-'a'+1))%mod;
        }
        return ;
    }
    
    static long getHash(String s,int n){
        // similar to number system 
        long p=1;
        long h=0;
        for(int i=0;i<n;i++){
            h=((h*37+(s.charAt(i)-'a'+1)))%mod;
