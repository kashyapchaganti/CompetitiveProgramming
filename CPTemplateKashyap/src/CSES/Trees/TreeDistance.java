...
class TreeDistance {
    ...
    static void check(int i, int p, List<List<Integer>> adj, int[] outDp, int[]inDp,int[] ans){
        ...
        for(int j=1;j<pSum.size();j++){
            pSum.set(j,Math.max(pSum.get(j-1),pSum.get(j)));
        }
        for(int j=pSum.size()-2;j>=0;j--){
            sSum.set(j,Math.max(sSum.get(j+1),sSum.get(j)));
        }
        int childNo=0;
        for(int x: adj.get(i)){
            if(x!=p){
                int left = childNo==0?0: pSum.get(childNo-1);
                int right = childNo==sSum.size()-1?0: sSum.get(childNo+1);
                
                if(left!=0){
                    outDp[x]=Math.max(outDp[x],1+left);
                }
                if(right!=0){
                    outDp[x]=Math.max(outDp[x],1+right);
                }
                outDp[x]=Math.max(1+outDp[i],outDp[x]);
                check(x,i,adj,outDp,inDp,ans);
                childNo++;
                
            }
            
        }
        //out.println(i+" "+inDp[i]+" "+outDp[i]);
        ans[i]=Math.max(inDp[i],outDp[i]);
        return;
    }
