...
class TreeMatching {
    ...
    static void check(int[][]dp, int i, int p,List<List<Integer>> adj ){
        ...
        for(int x: adj.get(i)){
            if(x!=p){
                pSum.add(Math.max(dp[x][0],dp[x][1]));
                sSum.add(Math.max(dp[x][0],dp[x][1]));
            }
        }
        for(int j=1;j<pSum.size();j++){
            pSum.set(j,pSum.get(j-1)+pSum.get(j));
        }
        for(int j=sSum.size()-2;j>=0;j--){
            sSum.set(j,sSum.get(j)+sSum.get(j+1));
        }
        dp[i][0]= pSum.get(pSum.size()-1);
        int childNo=0;
        for(int x: adj.get(i)){
            if(x==p) continue;
            int leftChild = childNo==0?0: pSum.get(childNo-1);
            int rightChild = childNo==sSum.size()-1?0: sSum.get(childNo+1);
            dp[i][1]=Math.max(dp[i][1],1+leftChild+rightChild+dp[x][0]);
            childNo++;
        }
        maxi=Math.max(dp[i][0],dp[i][1]);
