...
class TreeDistanceTwo {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (System.getProperty("ONLINE_JUDGE") == null && !ONLINE_JUDGE) {
                FileOutputStream output = new FileOutputStream("src/CSES/Trees/output.txt");
                PrintStream out = new PrintStream(output);
                System.setOut(out);
 
                InputStream input = new FileInputStream("src/CSES/Trees/input.txt");
                System.setIn(input);
            }
        ...
        try {
            ...
            depth(1,-1,adj,child,inDp);
            inCal(1,-1,adj,inDp,child);
            int[] outDp = new int[n+1];
            int[] ans = new int[n+1];
            check(1,-1,adj,outDp,inDp,ans,child, n);
            ...
            //check(dp,1,-1,adj);
            //out.println(adj);
            //out.println(Arrays.toString(inDp));
            //out.println(Arrays.toString(child));
    ...
    static void inCal(int i, int p, List<List<Integer>> adj, int[]inDp, int[] child){
        
        for(int x: adj.get(i)){
            if(x!=p){
                inCal(x,i,adj,inDp, child);
                int a =inDp[x]+1+child[x];
                inDp[i]+=a;
            }
        }
       
    }
    static void check(int i, int p, List<List<Integer>> adj, int[] outDp, int[]inDp,int[] ans,int[] child,int n){
        List<Integer> pSum= new ArrayList<>();
        List<Integer> sSum= new ArrayList<>();
        int totalNodesSubTree=0;
        for(int x: adj.get(i)){
            if(x!=p){
                //check(x,i,adj,outDp, inDp,ans, child, n);
                int a =inDp[x]+1+child[x];
                pSum.add(a);
                sSum.add(a);
                totalNodesSubTree+=child[x];
                //inDp[i]+=a;
            }
        }

        for(int j=1;j<pSum.size();j++){
            pSum.set(j, pSum.get(j-1)+pSum.get(j));
        }
        int n1 = pSum.size();
        for(int j=n1-2;j>=0;j--){
            sSum.set(j, sSum.get(j+1)+sSum.get(j));
        }
        int c_no=0;
        for(int x: adj.get(i)){
            if(x!=p){
                //other children
                outDp[x]=(totalNodesSubTree-(1+child[x]))+(c_no==0?0:pSum.get(c_no-1))+(c_no==n1-1? 0:sSum.get(c_no+1));
                //from the out of parent
                outDp[x]+=1+outDp[i]+(n-(1+totalNodesSubTree));
        ...
        //out.println(outDp[i] +" " +i);
        ans[i]=outDp[i]+inDp[i];
        return;
    }
    static void depth(int i, int p, List<List<Integer>> adj, int[] child,int[] inDp ){
        
        //inDp[i]=0;
        for(int x: adj.get(i)){
            if(x!=p){
                depth(x,i,adj,child,inDp);
                child[i]+=1+child[x];
                //inDp[i]=Math.max(inDp[i],1+inDp[x]);
            }
        }
