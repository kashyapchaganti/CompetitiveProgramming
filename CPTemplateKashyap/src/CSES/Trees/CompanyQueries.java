...
class CompanyQueries {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            binaryLifting(1,-1,dp,adj);
            while(q-->0){
                int x1 = sc.nextInt();
                int x2 = sc.nextInt();
                int ans = query(x1,x2,dp);
                out.println(ans);
            }
            //out.println(maxi);
    ...
    static int query(int x1, int x2, int[][] dp){
        if(x1==-1 || x2==0){
            return x1;
        }
        for(int i=19;i>=0;i--){
            if(x2>=(1<<i)){
                int x3= dp[x1][i];
                return query(x3,(x2-(1<<i)),dp);
            }
        }
        return -1;
    }
    static void binaryLifting(int i, int p, int[][] dp ,List<List<Integer>> adj){
        dp[i][0]=p;
        for(int j=1;j<20;j++){
            if(dp[i][j-1]!=-1){
