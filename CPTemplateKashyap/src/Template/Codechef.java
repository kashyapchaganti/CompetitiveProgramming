...
class Codechef {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (System.getProperty("ONLINE_JUDGE") == null && !ONLINE_JUDGE) {
                FileOutputStream output = new FileOutputStream("src/Template/output.txt");
                PrintStream out = new PrintStream(output);
                System.setOut(out);
 
                InputStream input = new FileInputStream("src/Template/input.txt");
                System.setIn(input);
            }
    ...
    static class Utility {
        ...
        static void generateSubseqsuences(List<List<Integer>> subs,List<Integer> x,int[] nums, int i){
            if(i==nums.length){
                subs.add(new ArrayList<>(x));
                return;
            }
            for(int k=i;k<nums.length;k++){
                x.add(nums[k]);
                generateSubseqsuences(subs,x,nums,k+1);
                x.remove(x.size()-1);
            }
    ...
    static class SegmentTree{
        ...
        public int query(int i, int l, int r, int low, int high){
            if(r<low || l>high ) return (int)(1e9);
